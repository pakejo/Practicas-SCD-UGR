...
#define etiq_esta_malafolla 4

const int SACIADO = 5;
const int HAMBREINTO = 6;

const int
    id_camarero = 10,
    id_cliente_malafolla = 4,
    num_filosofos = 5,
...
void funcion_filosofos(int id)
{
    ...
    bool puede_comer = true;
    int esta_filosofo_malafolla = 0;
    int estado_filosofo = HAMBREINTO;
    ...
    while (true)
    {
        ...
        // Preguntamos si esta el filosofo malafolla
        if (id != 4)
            MPI_Recv(&esta_filosofo_malafolla, 1, MPI_INT, id_camarero, etiq_esta_malafolla, MPI_COMM_WORLD, &estado);

        if ((id != 4) && (esta_filosofo_malafolla = 1)) // Si es un filosofo amable
        {
            if (estado_filosofo == HAMBREINTO)
                puede_comer = true;
            else
                puede_comer = false;
        }
        else if ((id != 4) && (esta_filosofo_malafolla = 0))
            puede_comer = true;

        if (puede_comer || (id == 4)) // El filosofo malafolla siempre come
        {
...
void funcion_camarero()
{
    ...
    while (true)
    {
        ...
        case etiq_sentarse:
            ...
            cout << "Peticion filosofo: " << peticion_filosofo << endl;
            ...
            {
                // Decimos al filosofo si esta el malafolla
                MPI_Ssend(&esta_malafolla, 1, MPI_INT, status.MPI_SOURCE, etiq_esta_malafolla, MPI_COMM_WORLD);
            }
            else
            {
                // Recibimos la peticion del filosofo malafolla
                esta_malafolla = 1;
                cout << "El filosofo malafolla se sienta" << endl;
            }
        ...
        case etiq_levantarse:
            ...
            // Se va el filosofo malafolla
            if (status.MPI_SOURCE == 4)
            {
                cout << "Se va el filosofo malafolla" << endl;
                esta_malafolla = 0;
            }
